...
public class SingleUseECPreKeysPostgres implements SingleUseECPreKeyStorage {
  ...
  static Void store(final Connection connection, final UUID identifier, final byte deviceId, final List<ECPreKey> preKeys) throws SQLException {
    final List<ECPreKey> snapshot = List.copyOf(preKeys);
  ...
  static Optional<ECPreKey> take(final Connection connection, final UUID identifier, final byte deviceId) throws SQLException {
    try (var statement = connection.prepareStatement("""
