...
public final class ProfilesPostgres implements ProfileDataStore {
  ...
  private static void writeV1(Connection connection, UUID account, VersionedProfileV1 p) throws SQLException {
  ...
  @Override public Optional<VersionedProfileV1> getV1(UUID account, String version) {
    try (var connection = dataSource.getConnection(); var statement = connection.prepareStatement(
        "SELECT * FROM signal.profiles_v1 WHERE account_id=? AND version=?")) {
      statement.setObject(1, account);
  ...
  private static Optional<VersionedProfile> getV2(Connection connection, UUID account, byte[] version) throws SQLException {
  ...
  @Override public VersionedProfileV1 setV1Avatar(UUID account, String version, String avatar, byte[] commitment) {
    return uncontestedTransaction(account, connection -> {
      try (var statement = connection.prepareStatement("""
