...
`SingleUseECPreKeysPostgres` and `SingleUseKEMPreKeysPostgres` preserve the original public-key encodings and KEM signatures. Their transactions coordinate bulk replacement, consumption and account/device deletion across store instances. Invalid writes roll back complete batches; malformed stored keys fail without consuming them. KEM exhaustion falls back to the repeated-use signed KEM store through `KeysManager`. The native one-time KEM path has no object-storage pages.

The legacy DynamoDB EC and repeated-use signed-key stores, the DynamoDB/S3 paged KEM store, their page codec and their key-specific configuration have been removed. Server and worker composition now select native PostgreSQL keys directly. The provider-neutral key interfaces remain; no cryptographic encoding, signature verification or key-consumption contract is replaced. Dead page-listing/pruning APIs and their exclusive fixtures are removed with the backend.

Account/key lifecycle integration fixtures use a fresh disposable PostgreSQL database per test, with account, key, recovery and lock implementations selected together. They preserve phone-account creation/re-registration/deletion, device linking/removal, number changes, username and recovery migration coverage. Receipt-based numberless success cases are replaced with explicit unsupported-operation and no-persisted-account/key assertions, matching the existing pilot limitation.

The key cleanup passed **254 local tests** with no failures, errors or skips in the combined key/account/configuration/server/admission run, including the 16-case admission-account creator suite developed alongside it. Evidence is the root workspace's `.local/aws-keys-cleanup-tests.log`; these are overlapping regression suites, not 254 newly added tests. This source checkpoint reduces AWS-importing main Java files from 38 to 33. The runtime classpath still includes 38 AWS jars (36,330,218 bytes). A subsequent clean production compile and actual CLI help check passed, and all eight removed key implementation/configuration/page classes are absent from compiled output. No cloud migration or deployment is part of this cleanup.
...
Profile metadata now uses PostgreSQL in both server and worker composition. The obsolete `Profiles`, `ProfilesV2`, `DynamoProfileDataStore` and `ProfileAvatars` implementations, Dynamo table settings and exclusive backend fixtures are removed. `ProfilesManager` accepts only `ProfileDataStore`, `ProfileAvatarStore` and `AvatarObjectStorage`; it no longer imports AWS types or exposes S3-specific constructors. Manager tests retain cache fallback, cache invalidation, deletion, missing-object and refresh-retry behavior through those interfaces. Native PostgreSQL tests cover independent profile versions, immutable commitments, conditional updates, atomic rollback/concurrency, payment-address/hash replacement and avatar metadata. The legacy runtime still explicitly supplies `S3AvatarObjectStorage` at its composition boundary; GCP mode supplies the existing GCS implementation. This bounded cleanup does not remove the remaining S3 policy/object adapters or AWS SDK dependencies and performs no cloud deployment.

The profile cleanup passed **244 local tests** with no failures, errors or skips: native PostgreSQL profile storage, profile manager/cache behavior, REST and authenticated/anonymous gRPC profiles, GCS avatar behavior, configuration/worker checks, and the full isolated server fixture. Evidence is the root workspace's `.local/aws-profiles-cleanup-tests.log`. At this source checkpoint, 38 main Java files still import AWS SDK types; the runtime classpath still includes 38 AWS jars. These are separate source-file and dependency counts, not a claim that the artifact is AWS-free.
...
The obsolete DynamoDB `AppleDeviceChecks` implementation and its two table configurations/fixture definitions are removed. The optional legacy App Attest feature now selects `AppleDeviceChecksPostgres` directly; `GCP_PILOT` continues to exclude the feature. The provider-neutral store contract and unchanged credential/CBOR codec retain global public-key ownership, equal-or-increasing counters and transaction rollback on ownership conflicts. Manager and native-store tests use a fresh database per case in a disposable PostgreSQL container, without connecting to the operator database or requiring environment flags to avoid skipping storage tests. The former Dynamo-only test is removed after its storage contracts are retained by the broader native suite.

The DeviceCheck cleanup passed **65 local tests** with no failures, errors or skips: 9 native storage, 7 manager, 9 controller, 12 configuration, 21 worker and 7 complete local server tests. Full production/test compilation and actual CLI help passed. The compiled-output audit confirmed that the deleted backend/test bytecode is absent. Evidence is `.local/aws-devicecheck-cleanup-tests.log` and `.local/aws-devicecheck-cleanup-cli-help.log` in the root workspace. At this checkpoint, 32 production files import AWS SDK types and 38 AWS runtime jars remain; no cloud deployment or App Attest feature enablement is implied.
...
## S3 avatar and sticker removal, September 22, 2026

The obsolete S3 avatar object provider, upload-policy adapter/signer, CDN configuration, REST sticker controller and exclusive sticker entity are removed. Both server and supported worker composition now require the existing GCS avatar provider. Any explicit legacy `cdn` configuration, including null, is rejected even by permissive configuration mappers. Profile REST/authenticated/anonymous gRPC constructors accept only the provider-neutral policy interface; the retained protocol upload-form shape does not imply an S3 provider. GCS object lifecycle, IAM signing, attachment/private-media paths and current membership guards remain intact. Sticker REST upload routes are absent and the retained sticker RPC returns structured `UNAVAILABLE` without signing or consuming attachment rate limits. The unused sticker limiter and LocalStack S3 fixture/dependency are also removed.

The final local regression suite passed **446 tests, zero failures/errors/skips**, including native PostgreSQL 18 profile storage, provider-neutral profile transport behavior, GCS object/signing/private downloads, attachment guards, eight complete local server cases, Stories REST/gRPC and APNs. The full-server fixture intercepts ADC and object-storage construction explicitly; it never uses operator credentials or contacts GCP for avatars. Test evidence is `.local/aws-avatar-sticker-cleanup-tests.log` in the root workspace. A subsequent clean production compile and compiled CLI check passed; all six removed production classes are absent. The `server`, message persister and scheduled APNs sender commands remain registered.

Fresh runtime dependency resolution measures **22 production Java files importing AWS types** (previously 23) and **35 AWS runtime JARs / 31,839,350 bytes** (previously 38 / 36,330,218). The removed JARs are `s3`, `aws-xml-protocol` and `arns`. DynamoDB, STS, CRT and shared AWS types remain, including the static credentials SPI still used by legacy Dynamo fixtures. Evidence: `.local/aws-avatar-sticker-clean-compile.log`, `.local/aws-avatar-sticker-cli-help.log`, `.local/aws-avatar-sticker-cleanup-audit.json` and the refreshed `.local/signal-runtime-classpath.txt`. This checkpoint changes source and clean local build outputs; no image rebuild or cloud rollout is implied.
